...
export default function Register() {
    ...
    const validationSchema = Yup.object().shape({
        username: Yup.string().required("Username is required").max(30, "Maximum length is 30 characters"),
        password: Yup.string().required("Password is required").max(30, "Maximum length is 30 characters"),
        passwordConfirm: Yup.string()
            .required("Confirm password is required")
            .oneOf([Yup.ref("password"), null], "Confirm password does not match"),
        name: Yup.string().required("Username is required").max(30, "Maximum length is 30 characters")
    });
        ...
        <>
            <AppBar position="static">
                <Toolbar>
                    ...
                    <div>
                        <Button
                            className={`${stylesNavBar.btnMaterial} ${stylesNavBar.btnCreate}`}
                            onClick={() => navigate("/")}
                        >
                            LOGIN
                        </Button>
            ...
            <div className={styles.container}>
                ...
                <form className={`${styles.container} ${styles.containerForm}`} onSubmit={handleSubmit(registerUser)}>
                            ...
                            endAdornment: (
                                <InputAdornment position="end">
                                    <IconButton
                                        ...
                                        edge="end"
                                    >
                                        {isPasswordShowed ? <Visibility /> : <VisibilityOff />}
                                    </IconButton>
                                </InputAdornment>
                            ),
                            maxLength: 30
                            ...
                            endAdornment: (
                                <InputAdornment position="end">
                                    <IconButton
                                        ...
                                        edge="end"
                                    >
                                        {isPasswordConfirmShowed ? <Visibility /> : <VisibilityOff />}
                                    </IconButton>
                                </InputAdornment>
                            ),
                            maxLength: 30
                    ...
                    />
                    <Button className={`${styles.btnRegister}`} variant="contained" type="submit">
                        Register
                    </Button>
                </form>
            </div>
